 ...
 * ====
 */
contract VAULT_B is ERC20, IERC4626 {
    ...
    modifier onlyController() {
        require(msg.sender == controller, "VaultB: Not controller");
        _;
    }

    modifier notLocked(address user) {
        require(
            !VAULT_CONTROLLER(controller).isLocked(user),
            "User is locked due to an active loan"
        );
        _;
    }
    ...
    function deposit(
        uint256 assets,
        address receiver
    ) public virtual notLocked(receiver) returns (uint256) {
    ...
    function withdraw(
        ...
        address receiver,
        address owner
    ) public virtual notLocked(owner) returns (uint256) {
